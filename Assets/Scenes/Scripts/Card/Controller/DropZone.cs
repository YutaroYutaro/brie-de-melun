...
public class DropZone : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public async void OnDrop(PointerEventData eventData)
    {
        ...
        if (PhaseManager.Instance.GetNowPhase() != "SelectUseCard") return;

        GameObject dragGameObject = eventData.pointerDrag;
        Draggable dragGameObjectDraggable = dragGameObject.GetComponent<Draggable>();

        if (dragGameObjectDraggable == null) return;

        Debug.Log(eventData.pointerDrag.name + " was dropped on " + name);
